...
- 电机分配为 M1 右侧行驶、M2 左侧行驶、M3 送牌、M4 出牌。M3/M4 上电后默认关闭，网页可分别实时设置 PWM（1–255）和运行时间（0.1–60 秒）；默认值为 M3 255/5 秒、M4 255/1 秒。网页保持 `W/A/S/D` 行驶控制；单击键盘 `P` 会通过同一按键控制通道触发 M4，并在 Arduino 确认后显示结果。
