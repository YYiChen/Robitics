#!/usr/bin/env bash
# Isolated whole-drive check: M1/M2 stay at +90; camera, vision and card controls are untouched.
set -Eeuo pipefail
...
echo "Constant drive active: right=90 left=90. Card controls are untouched. Ctrl+C stops M1/M2."
python3 -u "${HERE}/constant_drive_runner.py" --controller-url "${CONTROLLER_URL}" --pwm 90 --command-hz 10
