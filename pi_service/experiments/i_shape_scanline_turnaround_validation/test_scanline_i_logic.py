...
def planner_evidence(*, endpoint=False, endpoint_y=360, lost=False, junction_y=None, height=480):
    ...
    return ScanlineEvidence(
        ...
        endpoint_y=endpoint_y if endpoint else None,
...
class HybridScanlineAnalyzerTests(unittest.TestCase):
    ...
    def test_early_prediction_authorizes_one_endpoint_frame(self):
        ...
        self.assertIs(planner.step(planner_evidence(endpoint=True, endpoint_y=300), 0.1).state, TurnaroundState.BAR_MARKED)
    ...
    def test_near_endpoint_authorizes_fast_lost_frame_when_junction_is_far(self):
        """A near confirmed bar must not lose fast authorization to its higher junction."""
        planner = IShapeTurnaroundPlanner(
            TurnaroundConfig(junction_confirm_frames=1, endpoint_confirm_frames=2, line_lost_confirm_frames=3, early_junction_trigger_y_ratio=0.75, early_line_lost_confirm_frames=1)
        )
        self.assertIs(planner.step(planner_evidence(junction_y=324), 0.0).state, TurnaroundState.EARLY_BAR_PREDICTED)
        self.assertIs(planner.step(planner_evidence(endpoint=True, endpoint_y=363), 0.1).state, TurnaroundState.BAR_MARKED)
        self.assertIs(planner.step(planner_evidence(lost=True), 0.2).state, TurnaroundState.BRAKE_BEFORE_PIVOT)
