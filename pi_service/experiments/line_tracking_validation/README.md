...
普通循迹不再只选择固定的 `FL/FR` 档位。它使用图像中的标准化偏差 `offset` 计算 `修正量 = 450 × |offset|`：线偏右时左轮加速、右轮减速；线偏左时相反。两轮 PWM 被限制在内侧 60、外侧 180 之间，线接近画面中线（`|offset| <= 0.05`）时两轮同为直行 PWM 110。直角转向仍使用固定的 `PR=155`，不会受 P 微调干扰。
