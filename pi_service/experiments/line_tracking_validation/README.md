...
- 启动脉冲：从静止起步时 `SF` 为 145、持续约 0.18 秒；
- 低速巡航：随后 `F` 为 85；
...
普通循迹不再只选择固定的 `FL/FR` 档位。它使用地面坐标中的标准化偏差 `offset` 计算 `修正量 = 150 × |offset|`：线偏右时左轮加速、右轮减速；线偏左时相反。两轮 PWM 被限制在内侧 60、外侧 180 之间，线接近画面中线（`|offset| <= 0.05`）时两轮同为巡航 PWM 85；每次 PWM 最多变化 12。直角转向仍使用固定的 `PR=155`，不会受 P 微调干扰。

启动功率和巡航功率分开：每次从停止重新进入前进时，先以 145 PWM 运行约 0.18 秒以克服静摩擦，再自动降为 85 PWM。这让车可以起步，同时给视觉控制留下更慢、更可预测的巡航速度。
