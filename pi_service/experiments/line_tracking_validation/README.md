...
这是一个**无岔路、顺时针矩形**的固定路线。到达角落时，摄像头会比车身更早看不见旧边的黑线；因此先以 `F` 继续前进 6 个处理帧（10 FPS 时约 0.6 秒）。若期间重新看到线，就取消转弯；若仍失线，即使右侧横线因视野、反光或阈值原因没有被 `RIGHT BRANCH` 检测到，规划器才会进入 `TURN_RIGHT`，电机执行 `PR`（原地右转，PWM 120）。它会持续转动，直到连续三帧确认看见新的边线后才恢复前进。
