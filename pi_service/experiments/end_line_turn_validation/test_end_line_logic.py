...
class EndLineLogicTests(unittest.TestCase):
    ...
    def test_confirmed_white_line_loss_at_red_terminal_stops(self):
        planner = EndLineStopPlanner(EndLineConfig(line_lost_confirm_frames=2))
        first = planner.step(line_valid=False, red_detected=True)
        decision = planner.step(line_valid=False, red_detected=True)
        self.assertFalse(first.stop)
        self.assertTrue(decision.stop)
        self.assertEqual(decision.state, EndLineState.STOPPED_LINE_END)
